...
- **По умолчанию**: установка в папку рядом с `Installer.exe` (всё в одном месте).
- **Если нет прав на запись** (например `Program Files`): установщик попросит права администратора (UAC).
- **Можно принудительно**: установка в `%LOCALAPPDATA%\journal-parser\` или `Program Files` через параметры `install.ps1` (для разработчика).
